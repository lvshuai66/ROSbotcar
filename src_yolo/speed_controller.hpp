...
namespace nav2_behavior_tree
{
  ...
  public:
    ...
    rclcpp::Subscription<Detection2DArray>::SharedPtr yolo_result_sub_;
    void yoloResultCallback(const Detection2DArray::SharedPtr msg)
    {
      std::map<std::string, double> result_dict;
      for (auto &detection : msg->detections)
      {
        for (auto &result : detection.results)
        {
          result_dict[result.id] = std::round(result.score * 10000) / 10000.0;
        }
      }
      if (result_dict.find("bottle") != result_dict.end())
      {
        yolo_result_ = 0.0;
      }
      else if (result_dict.find("person") != result_dict.end())
      {
        yolo_result_ = 0.2;
      }
      else
