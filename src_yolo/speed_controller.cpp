...
namespace nav2_behavior_tree
{
        ...
        max_speed_(0.5)
  {
    ...
    yolo_result_sub_ = node_->create_subscription<Detection2DArray>(
        "yolo_result", 10,
        std::bind(&SpeedController::yoloResultCallback, this, std::placeholders::_1));
  ...
  inline BT::NodeStatus SpeedController::tick()
  {
        ...
        elapsed.seconds() >= period_)
    {
      ...
      if (elapsed.seconds() >= period_)
      {
        updatePeriod();
        start_ = node_->now();
        // Update speed based on yolo_result_
        max_speed_ = origin_speed_*yolo_result_;
      }
