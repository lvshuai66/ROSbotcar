...
class YoloV5Ros2(Node):
    def __init__(self):
        ...
        # /camera/image_raw
        self.declare_parameter("video_url", "http://192.168.31.100:81/stream", ParameterDescriptor(
            name="video_url", description=f"default: http://192.168.31.100:81/stream"))
        ...
        self.cap = cv2.VideoCapture(self.get_parameter('video_url').value)
        if not self.cap.isOpened():
            raise Exception("Could not open video stream")

        self.timer_period = 1.0 / 60.0  # 30fps
    ...
    def image_callback(self):
        ...
        # 5. check if frame is None (reached end of video stream)
        if not ret:
            return

        # 5.detect pub result
        # image = self.bridge.imgmsg_to_cv2(msg)
        detect_result = self.yolov5.predict(frame)
